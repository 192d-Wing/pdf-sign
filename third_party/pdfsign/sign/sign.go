...
func (context *SignContext) SignPDF() error {
	...
	// Fork change (192d-Wing/pdf-sign): when SignatureField names an
	// existing empty /FT /Sig field, sign into it — the widget is already
	// on the page and in the AcroForm, so no new annotation is created.
	if context.SignData.SignatureField != "" {
		if err := context.signIntoExistingField(); err != nil {
			return err
		}
	} else {
	...
	// Fork addition (192d-Wing/pdf-sign): fill existing text fields in
	// this same incremental update so the values are covered by the
	// signature and earlier signatures stay valid.
	if err := context.fillTextFields(); err != nil {
		return fmt.Errorf("failed to fill fields: %w", err)
	}
